...
The generator outputs to both `output/` (for local use) and `postman/` (for future workspace push compatibility).

### Current Approach: API Uploader (Stable)
...
node src/api-uploader.js upload
node src/api-uploader.js upload-env
```

This uses our custom uploader which reliably handles:
- Collection format cleaning for Postman API
- Create or update logic
- Proper error handling

### Future Approach: Workspace Push (BETA - Experimental)

> ⚠️ **Note**: `postman workspace push` is currently in BETA and has known issues with creating new collections from externally generated files. The `prepare` and `push` commands have inconsistent validation logic. Use with caution.

```bash
# Create .postman/config.json first
mkdir -p .postman
...
# Prepare (validates and regenerates IDs)
...
**Why we keep the `postman/` directory structure:**
- Ready for when `workspace push` exits BETA
- Follows Postman's recommended directory layout
- Allows easy migration once the feature stabilizes
- Files are automatically generated by `src/index.js`

**Known Issues with `workspace push` (BETA):**
- Inconsistent validation between `prepare` and `push` commands
- Transformer errors on externally generated collections
- Environment ID handling issues
- Designed primarily for "workspace-as-code" workflows (pull → edit → push), not import workflows
